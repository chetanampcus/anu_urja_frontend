    ...
    <div className="flex bg-slate-50 min-h-screen overflow-hidden">
              ...
              <div key={item.href} className="px-3">
                <Link
                  ...
                  className={`px-3 py-3 rounded-xl flex items-center transition-all duration-200 group ${isActive
                      ? "bg-indigo-600/20 text-indigo-300 font-semibold border border-indigo-500/20 shadow-inner"
                      : "text-slate-400 hover:bg-white/5 hover:text-slate-200"
                    } ${collapsed ? "justify-center" : "gap-3"}`}
                  title={collapsed ? item.label : ""}
                >
                  <Icon className={`w-5 h-5 shrink-0 ${isActive ? "text-indigo-400" : "text-slate-400 group-hover:text-slate-300"} transition-colors`} />
      ...
      >
        <div className="flex items-center justify-between h-20 px-6 border-b border-indigo-900/50">
          ...
          <button
            className="p-2 -mr-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors focus:outline-none"
      ...
      {/* Page content */}
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto overflow-x-hidden bg-slate-50 relative">
        {/* Mobile top bar */}
        <div className="md:hidden sticky top-0 z-30 flex items-center bg-white/80 backdrop-blur-md border-b border-slate-200 text-slate-800 p-3 h-16 shadow-sm">
          <button
